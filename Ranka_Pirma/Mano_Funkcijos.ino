...
void rusiavimas(int spalvos_kodas){
  if(spalvos_kodas !=0){
   ...
   belt_off();
   delay(500); 
   movejoint(rusiavimo_point[0], rusiavimo_point[1], rusiavimo_point[2], rusiavimo_point[3]);
   ProtocolProcess();
   delay(2000);
   
   moveArm(rusiavimo_paemimas[0], rusiavimo_paemimas[1], rusiavimo_paemimas[2]+50, rusiavimo_paemimas[3]);
   ProtocolProcess();
   delay(2000);

   moveArm(rusiavimo_paemimas[0], rusiavimo_paemimas[1], rusiavimo_paemimas[2], rusiavimo_paemimas[3]);
   ProtocolProcess();
   delay(3000);
   ...
   belt_on();
    while(true){
      rusiavimo_optika = digitalRead(rusiavimo_optikos_pin);
      if(rusiavimo_optika == LOW){ delay(4000);  break; }
    }
    belt_off();
  }
   movejoint(rusiavimo_darbinis[0], rusiavimo_darbinis[1], rusiavimo_darbinis[2], rusiavimo_darbinis[3]);
...
void nuskaitymas_spalvos(){
  ...
  for(int i=0; i<=2; i++){
      ...
      if(aptikta_spalva >0){ buvo_aptikta = true; break;}
  ...
  for(address = 1; address < 127; address++ )
  {
    ...
    if (error == 0)
    {
      ...
      if (address<16)
          ...
          nDevices++;
          if(address == antros_rankos_adresas){
            Serial.println("Antra ranka online");
            antra_ranka_dirba = true;
          }
...
void siuntimas_paketo(int adr){
  Wire.beginTransmission(adr);                    // Start communication with the slave
  Wire.write((uint8_t*)dataArray, ARRAY_SIZE * sizeof(int)); // Send the array
  Wire.endTransmission();                                   // End the transmission
  delay(100);
}
void paketo_konfiguravimas(int tipas){
  if(tipas == 5){
     dataArray[0] = 5;  
     dataArray[1] = 1;
     dataArray[2] = 0;   dataArray[3] = 0;  dataArray[4] = 0;  dataArray[5] = 0;  dataArray[6] = 0; dataArray[7] = 0;
  }
}
